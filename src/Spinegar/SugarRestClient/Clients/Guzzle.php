...
class Guzzle implements ClientInterface
{
    ...
    {
        $response = $this->getClient()->post('oauth2/token', [
            'json' => [
                'grant_type' => 'password',
                'client_id' => 'sugar',
                'client_secret' => '',
     ...
     */
    public function request($method, $endpoint, $parameters = [], $decode_json = true)
    {
        // Move to getClient?
     ...
     * Returns:  Returns an Array if successful, otherwise FALSE
     */
    public function getFile($endpoint, $destinationFile, $parameters = [])
    {
     ...
     * Returns:  Returns an Array if successful, otherwise FALSE
     */
    public function postFile($endpoint, $parameters = [])
    {
        $output = [];
